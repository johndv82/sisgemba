...
@section('titulo')
    Mantenimiento de Regímenes de Salud
@endsection
    ...
    <div class="col-md-12">
        <div class="card">
            <div class="card-header">
                <strong class="card-title">Mantenimiento de Regímenes de Salud</strong>
            </div>
            <div class="card-body">
                ...
                <div class="row form-group">
                    <div class="col-md-12">
                        <div class="table-responsive table--no-card m-b-30">
                            <table class="table table-borderless table-data3" id="tblRegimenSalud" data-toggle="tblRegimenSalud">
                                <thead>
                                <tr>
...
@section('modal')
    <div class="modal fade" id="modalRegistroRegimenSalud" tabindex="-1" role="dialog"
         aria-labelledby="mediumModalLabel" aria-hidden="true">
        <div class="modal-dialog modal-md" role="document">
            <div class="modal-content">
                <div class="modal-header">
                    <h5 class="modal-title" id="mediumModalLabel">Registro de Régimen de Salud</h5>
                    <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                        <span aria-hidden="true">&times;</span>
                    </button>
                </div>
                <div class="modal-body">
                    <form id="formMantenimientoRegimenSalud" name="formMantenimientoRegimenSalud" autocomplete="off" onsubmit="return false;">
                        @csrf
                        <div class="row form-group">
                            <div class="col-md-12">
    ...
    <script type="text/javascript">
        ...
        window.operateEvents = {
            'click .edit': function (e, value, row, index) {
                ...
                $("#modalRegistroRegimenSalud").modal('show');
            },
            'click .delete': function (e, value, row, index) {
                swal.fire({
                    ...
                    if (result.isConfirmed) {
                        ...
                        $.ajax({
                            url: "{{ route('deleteRegimenSalud', 'idReg') }}".replace('idReg', row.id),
                            method: "POST",
                            data: {_token: _token},
                            dataType: 'json',
        ...
        CargarTablaRegimenSalud = function(){
            let _token = $('input[name="_token"]').val();
            $.ajax({
                url: "{{ route('listadoRegimenSalud') }}",
                method: "POST",
                data: {_token: _token},
                dataType: 'json',
                success: function (response) {
                    $tabla = $("#tblRegimenSalud");
                    $tabla.bootstrapTable('destroy');
                    if(response.code === 200){
                        $tabla.bootstrapTable({data: response.regimenes_salud,
        ...
        $('#btnRegistrar').click(function(){
            //Ingresar / Editar
            if ($("#formMantenimientoRegimenSalud").valid()) {
                let _token = $('input[name="_token"]').val();
                $.ajax({
                    url: "{{ route('saveRegimenSalud') }}",
                    method: "POST",
                    data: {
                        id: idRegistro,
                        nombre: $('#nombre').val(),
                    ...
                    success: function (response) {
                        if (response.code === 200) {
                            ...
                            CargarTablaRegimenSalud();
                            //Ocultar Modal
                            $('#modalRegistroRegimenSalud').modal('hide');
                        }
                    }
                });
            }
        ...
        if ($("#formMantenimientoRegimenSalud").length > 0) {
            $.validator.addMethod("valueNotEquals", function (value, element, arg) {
                return arg !== value;
            });

            $('#formMantenimientoRegimenSalud').validate({
                rules: {
                    nombre: {
                        required: true,
                        maxlength: 250
