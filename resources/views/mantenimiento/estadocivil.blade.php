...
@section('titulo')
    Mantenimiento de Estados Civiles
@endsection
    ...
    <div class="col-md-12">
        <div class="card">
            <div class="card-header">
                <strong class="card-title">Mantenimiento de Estados Civiles</strong>
            </div>
            <div class="card-body">
                ...
                <div class="row form-group">
                    <div class="col-md-12">
                        <div class="table-responsive table--no-card m-b-30">
                            <table class="table table-borderless table-data3" id="tblEstadosCiviles" data-toggle="tblEstadosCiviles">
                                <thead>
                                <tr>
...
@section('modal')
    <div class="modal fade" id="registroEstadoCivil" tabindex="-1" role="dialog"
         aria-labelledby="mediumModalLabel" aria-hidden="true">
        <div class="modal-dialog modal-md" role="document">
            <div class="modal-content">
                <div class="modal-header">
                    <h5 class="modal-title" id="mediumModalLabel">Registro de Estado Civil</h5>
                    <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                        <span aria-hidden="true">&times;</span>
                    </button>
                </div>
                <div class="modal-body">
                    <form id="formMantenimientoEstadoCivil" name="formMantenimientoEstadoCivil" autocomplete="off" onsubmit="return false;">
                        @csrf
                        <div class="row form-group">
                            <div class="col-md-12">
    ...
    <script type="text/javascript">
        ...
        window.operateEvents = {
            'click .edit': function (e, value, row, index) {
                ...
                $("#registroEstadoCivil").modal('show');
            },
            'click .delete': function (e, value, row, index) {
                swal.fire({
                    ...
                    if (result.isConfirmed) {
                        ...
                        $.ajax({
                            url: "{{ route('deleteEstadoCivil', 'idReg') }}".replace('idReg', row.id),
                            method: "POST",
                            data: {_token: _token},
                            dataType: 'json',
        ...
        CargarTablaEstadosCiviles = function(){
            let _token = $('input[name="_token"]').val();
            $.ajax({
                url: "{{ route('listadoEstadoCivil') }}",
                method: "POST",
                data: {_token: _token},
                dataType: 'json',
                success: function (response) {
                    $tabla = $("#tblEstadosCiviles");
                    $tabla.bootstrapTable('destroy');
                    if(response.code === 200){
                        $tabla.bootstrapTable({data: response.estados_civiles,
        ...
        $('#btnRegistrar').click(function(){
            //Ingresar / Editar
            if ($("#formMantenimientoEstadoCivil").valid()) {
                let _token = $('input[name="_token"]').val();
                $.ajax({
                    url: "{{ route('saveEstadoCivil') }}",
                    method: "POST",
                    data: {
                        id: idRegistro,
                        nombre: $('#nombre').val(),
                    ...
                    success: function (response) {
                        if (response.code === 200) {
                            ...
                            CargarTablaEstadosCiviles();
                            //Ocultar Modal
                            $('#registroEstadoCivil').modal('hide');
                        }
                    }
                });
            }
        ...
        if ($("#formMantenimientoEstadoCivil").length > 0) {
            $.validator.addMethod("valueNotEquals", function (value, element, arg) {
                return arg !== value;
            });

            $('#formMantenimientoEstadoCivil').validate({
                rules: {
                    nombre: {
                        required: true,
                        maxlength: 250
