...
@section('titulo')
    Mantenimiento de Niveles de Estudios
@endsection
    ...
    <div class="col-md-12">
        <div class="card">
            <div class="card-header">
                <strong class="card-title">Mantenimiento de Niveles de Estudios</strong>
            </div>
            <div class="card-body">
                ...
                <div class="row form-group">
                    <div class="col-md-12">
                        <div class="table-responsive table--no-card m-b-30">
                            <table class="table table-borderless table-data3" id="tblNivelEstudios" data-toggle="tblNivelEstudios">
                                <thead>
                                <tr>
...
@section('modal')
    <div class="modal fade" id="modalRegistroNivelEstudios" tabindex="-1" role="dialog"
         aria-labelledby="mediumModalLabel" aria-hidden="true">
        <div class="modal-dialog modal-md" role="document">
            <div class="modal-content">
                <div class="modal-header">
                    <h5 class="modal-title" id="mediumModalLabel">Registro de Nivel de Estudios</h5>
                    <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                        <span aria-hidden="true">&times;</span>
                    </button>
                </div>
                <div class="modal-body">
                    <form id="formMantenimientoNivelEstudios" name="formMantenimientoNivelEstudios" autocomplete="off" onsubmit="return false;">
                        @csrf
                        <div class="row form-group">
                            <div class="col-md-12">
    ...
    <script type="text/javascript">
        ...
        window.operateEvents = {
            'click .edit': function (e, value, row, index) {
                ...
                $("#modalRegistroNivelEstudios").modal('show');
            },
            'click .delete': function (e, value, row, index) {
                swal.fire({
                    ...
                    if (result.isConfirmed) {
                        ...
                        $.ajax({
                            url: "{{ route('deleteNivelEstudios', 'idReg') }}".replace('idReg', row.id),
                            method: "POST",
                            data: {_token: _token},
                            dataType: 'json',
        ...
        CargarTablaNivelEstudios = function(){
            let _token = $('input[name="_token"]').val();
            $.ajax({
                url: "{{ route('listadoNivelEstudios') }}",
                method: "POST",
                data: {_token: _token},
                dataType: 'json',
                success: function (response) {
                    $tabla = $("#tblNivelEstudios");
                    $tabla.bootstrapTable('destroy');
                    if(response.code === 200){
                        $tabla.bootstrapTable({data: response.niveles_estudios,
        ...
        $('#btnRegistrar').click(function(){
            //Ingresar / Editar
            if ($("#formMantenimientoNivelEstudios").valid()) {
                let _token = $('input[name="_token"]').val();
                $.ajax({
                    url: "{{ route('saveNivelEstudios') }}",
                    method: "POST",
                    data: {
                        id: idRegistro,
                        nombre: $('#nombre').val(),
                    ...
                    success: function (response) {
                        if (response.code === 200) {
                            ...
                            CargarTablaNivelEstudios();
                            //Ocultar Modal
                            $('#modalRegistroNivelEstudios').modal('hide');
                        }
                    }
                });
            }
        ...
        if ($("#formMantenimientoNivelEstudios").length > 0) {
            $.validator.addMethod("valueNotEquals", function (value, element, arg) {
                return arg !== value;
            });

            $('#formMantenimientoNivelEstudios').validate({
                rules: {
                    nombre: {
                        required: true,
                        maxlength: 250
