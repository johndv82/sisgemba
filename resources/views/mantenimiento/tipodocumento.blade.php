    ...
    <div class="col-md-12">
        <div class="card">
            <div class="card-header">
                <strong class="card-title">Mantenimiento de Tipos de Documento</strong>
            </div>
            <div class="card-body">
                ...
                <div class="row form-group">
                    <div class="col-md-12">
                        <div class="table-responsive table--no-card m-b-30">
                            <table class="table table-borderless table-data3" id="tblTipoDocumento" data-toggle="tblTipoDocumento">
                                <thead>
                                <tr>
...
@section('modal')
    <div class="modal fade" id="modalRegistroTipoDocumento" tabindex="-1" role="dialog"
         aria-labelledby="mediumModalLabel" aria-hidden="true">
        <div class="modal-dialog modal-md" role="document">
            <div class="modal-content">
                <div class="modal-header">
                    <h5 class="modal-title" id="mediumModalLabel">Registro de Tipo de Documento</h5>
                    <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                        <span aria-hidden="true">&times;</span>
                    </button>
                </div>
                <div class="modal-body">
                    <form id="formMantenimientoTipoDocumento" name="formMantenimientoTipoDocumento" autocomplete="off" onsubmit="return false;">
                        @csrf
                        <div class="row form-group">
                            <div class="col-md-12">
...
@section('scripts')
    <script type="text/javascript">
            ...
            'click .delete': function (e, value, row, index) {
                swal.fire({
                    ...
                    if (result.isConfirmed) {
                        ...
                        $.ajax({
                            url: "{{ route('deleteTipoDocumento', 'idReg') }}".replace('idReg', row.id),
                            method: "POST",
                            data: {_token: _token},
                            dataType: 'json',
        ...
        CargarTablaTipoDocumento = function(){
            let _token = $('input[name="_token"]').val();
            $.ajax({
                url: "{{ route('listadoTipoDocumento') }}",
                method: "POST",
                data: {_token: _token},
                dataType: 'json',
                success: function (response) {
                    ...
                    if(response.code === 200){
                        $tabla.bootstrapTable({data: response.tipos_docs,
        ...
        $('#btnRegistrar').click(function(){
            //Ingresar / Editar
            if ($("#formMantenimientoTipoDocumento").valid()) {
                let _token = $('input[name="_token"]').val();
                $.ajax({
                    url: "{{ route('saveTipoDocumento') }}",
                    method: "POST",
                    data: {
                        id: idRegistro,
                        nombre: $('#nombre').val(),
                    ...
                    success: function (response) {
                        if (response.code === 200) {
                            ...
                            CargarTablaTipoDocumento();
                            //Ocultar Modal
                            $('#modalRegistroTipoDocumento').modal('hide');
                        }
                    }
                });
            }
        ...
        if ($("#formMantenimientoTipoDocumento").length > 0) {
            $.validator.addMethod("valueNotEquals", function (value, element, arg) {
                return arg !== value;
            });

            $('#formMantenimientoTipoDocumento').validate({
                rules: {
                    nombre: {
                        required: true,
                        maxlength: 250
