...
@section('titulo')
    Mantenimiento de Cargos Laborales
@endsection
    ...
    <div class="col-md-12">
        <div class="card">
            <div class="card-header">
                <strong class="card-title">Mantenimiento de Cargos Laborales</strong>
            </div>
            <div class="card-body">
                ...
                <div class="row form-group">
                    <div class="col-md-12">
                        <div class="table-responsive table--no-card m-b-30">
                            <table class="table table-borderless table-data3" id="tblCargosLaborales" data-toggle="tblCargosLaborales">
                                <thead>
                                <tr>
...
@section('modal')
    <div class="modal fade" id="modalRegistroCargoLaboral" tabindex="-1" role="dialog"
        ...
        <div class="modal-dialog modal-md" role="document">
            <div class="modal-content">
                ...
                <div class="modal-body">
                    <form id="formMantenimientoCargoLaboral" name="formMantenimientoCargoLaboral" autocomplete="off" onsubmit="return false;">
                        @csrf
                        <div class="row form-group">
                            <div class="col-md-12">
    ...
    <script type="text/javascript">
        ...
        window.operateEvents = {
            'click .edit': function (e, value, row, index) {
                ...
                $("#modalRegistroCargoLaboral").modal('show');
            },
            'click .delete': function (e, value, row, index) {
                swal.fire({
                    ...
                    if (result.isConfirmed) {
                        ...
                        $.ajax({
                            url: "{{ route('deleteCargoLaboral', 'idReg') }}".replace('idReg', row.id),
                            method: "POST",
                            data: {_token: _token},
                            dataType: 'json',
        ...
        CargarTablaCargosLaborales = function(){
            let _token = $('input[name="_token"]').val();
            $.ajax({
                url: "{{ route('listadoCargoLaboral') }}",
                method: "POST",
                data: {_token: _token},
                dataType: 'json',
                success: function (response) {
                    $tabla = $("#tblCargosLaborales");
                    $tabla.bootstrapTable('destroy');
                    if(response.code === 200){
                        $tabla.bootstrapTable({data: response.cargos_laborales,
        ...
        $('#btnRegistrar').click(function(){
            //Ingresar / Editar
            if ($("#formMantenimientoCargoLaboral").valid()) {
                let _token = $('input[name="_token"]').val();
                $.ajax({
                    url: "{{ route('saveCargoLaboral') }}",
                    method: "POST",
                    data: {
                        id: idRegistro,
                        nombre: $('#nombre').val(),
                    ...
                    success: function (response) {
                        if (response.code === 200) {
                            ...
                            CargarTablaCargosLaborales();
                            //Ocultar Modal
                            $('#modalRegistroCargoLaboral').modal('hide');
                        }
                    }
                });
            }
        ...
        if ($("#formMantenimientoCargoLaboral").length > 0) {
            $.validator.addMethod("valueNotEquals", function (value, element, arg) {
                return arg !== value;
            });

            $('#formMantenimientoCargoLaboral').validate({
                rules: {
                    nombre: {
                        required: true,
                        maxlength: 250
